...
def roc_curve_points(y_true, scores):
    ...
    for t in thresholds:
        ...
        tpr = tp / (tp + fn + 1e-8) if n_pos else 0.0
        fpr = fp / (fp + tn + 1e-8) if n_neg else 0.0
        tpr_list.append(tpr)
        fpr_list.append(fpr)
...
def plot_score_distributions(train_scores, records, threshold, out_path, score_label):
    test_good = [r["score"] for r in records if r["label"] == 0]
    test_defect = [r["score"] for r in records if r["label"] == 1]
    ...
    ax.hist(
        ...
        alpha=0.55,
        density=True,
        label=f"Test good, n={len(test_good)}",
        color="#3498db",
    )
    ax.hist(
        test_defect,
        ...
        alpha=0.55,
        density=True,
        label=f"Test defect, n={len(test_defect)}",
        color="#e74c3c",
    )
    ax.axvline(
        threshold,
